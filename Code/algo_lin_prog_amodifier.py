...
def solve_stage_problem(c1, A1, b1, cutting_planes):
    """
    ...
    if len(cutting_planes) == 0:  
        ...
        b_aug = np.concatenate((b1, b_cutting))
        #b_aug = np.vstack((b1, b_cutting))
        c_aug = np.concatenate((c1, [1]))  
...
while z_sup - z_min > epsilon and index < 10:
    ...
    if not res1.success:
     ...
     # z_min est le cout du probleme 1
    ...
    #res2= solve_stage_problem(c2, A2, (b2 - np.dot(E1, x1_hat)), [])
